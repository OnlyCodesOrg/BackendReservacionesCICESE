...
export class SalasService {
    ...
    validarDisponibilidadSala(
        ...
        if (conflictos.length > 0) {
            ...
            return {
                ...
                sugerencias: this.generarSugerenciasHorario(idSala, fechaEvento, reservasExistentes)
            };
        }
     ...
     */
    private hayTraslapeHorario(inicio1: Date, fin1: Date, inicio2: Date, fin2: Date): boolean {
    ...
    private generarSugerenciasHorario(
        ...
        if (reservasDelDia.length > 0) {
            ...
            if (primeraReserva.horaInicio.getHours() > 8) {
                alternativas.push(`08:00 - ${primeraReserva.horaInicio.toTimeString().substr(0, 5)}`);
            }
        }
        ...
        for (let i = 0; i < reservasDelDia.length - 1; i++) {
            ...
            const diferencia = (inicioSiguiente.getTime() - finActual.getTime()) / (1000 * 60); // minutos
            if (diferencia >= 60) { // Al menos 1 hora disponible
                alternativas.push(`${finActual.toTimeString().substr(0, 5)} - ${inicioSiguiente.toTimeString().substr(0, 5)}`);
            }
        }
        ...
        if (reservasDelDia.length > 0) {
            ...
            if (ultimaReserva.horaFin.getHours() < 18) {
                alternativas.push(`${ultimaReserva.horaFin.toTimeString().substr(0, 5)} - 18:00`);
            }
        }
